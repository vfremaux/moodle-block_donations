...
class block_donations extends block_list {
	...
	function specialization() {
		...
		if (empty($this->config->numberofdonations)) {
			$title->config->numberofrecords = count_records('donationsingle', 'course', $COURSE->id);
		}
    ...
    function get_content() {
		...
		if ($this->config->localcourse){
			$select = " course = $COURSE->id AND globalview = 1 AND deadline < $now ";
		} else {
			$select = " globalview = 1 AND deadline > $now ";
		}
		$donations = get_records_select('donationsingle', $select, $sort, '*', 0, $limit); 

		foreach($donations as $donation) { 
			
			$coursecontext = get_context_instance(CONTEXT_COURSE, $donation->course);
			$courseenrollable = get_field('course', 'enrollable', 'id', $donation->course);
    ...
    /// set up the numberoftags select field
		...
		$limit = count_records('donationsingle', 'course', $COURSE->id);
